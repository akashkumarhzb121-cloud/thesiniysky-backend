...
app.get('/api/v1/testimonials/all', async (req, res) => { try { const items = await require('./models/Testimonial').find().sort({createdAt:-1}).lean(); res.json({success:true,data:items}); } catch(e) { res.status(500).json({success:false,message:e.message}); } });
...
app.post('/api/v1/blogs/:id/comments', protect, async (req, res) => {  try {    const Blog = require('./models/Blog');    const blog = await Blog.findById(req.params.id);    if (!blog) return res.status(404).json({success:false,message:'Blog not found'});    blog.comments = blog.comments || [];    blog.comments.push({ user: req.user.id, content: req.body.content, createdAt: new Date() });    await blog.save();    const populated = await Blog.findById(req.params.id).populate('comments.user', 'firstName lastName email').lean();    res.json({success:true,data:populated});  } catch(e) { res.status(500).json({success:false,message:e.message}); }});
...
app.delete('/api/v1/invoices/:id', protect, authorize('super_admin','admin'), async (req, res) => { try { await Invoice.findByIdAndDelete(req.params.id); res.json({success:true,message:'Invoice deleted'}); } catch(e) { res.status(500).json({success:false,message:e.message}); } });
