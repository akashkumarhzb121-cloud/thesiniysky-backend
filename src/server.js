...
app.post('/api/v1/blogs/:id/comments', protect, async (req, res) => {
  try {
    ...
    blog.comments.push({ user: req.user.id, content: req.body.content, createdAt: new Date() });
    await blog.save();
    const populated = await Blog.findById(req.params.id).populate('comments.user', 'firstName lastName email').lean();
